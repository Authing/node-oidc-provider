...
module.exports = function getClient(provider) {
  ...
  async function buildKeyStore(client) {
    ...
    if (client.customJwks) {
      try {
        let key = JSON.parse(client.customJwks)
        if (handled(key.kty)) {
          promises.push(client.keystore.add(key));
        }
      } catch(e) {}
      // client.customJwks.keys.forEach((key) => {
      //   if (handled(key.kty)) {
      //     promises.push(client.keystore.add(key));
      //   }
      //   return undefined;
  ...
  class Client {
    ...
    redirectUriAllowed(redirectUri) {
      ...
      let exps = this.redirectUris.map(item => new RegExp(item))
      let ok = false;
      for(let i=0;i<exps.length;i++) {
        if(exps[i].test(checkedUri)) {
          ok = true;
          break;
        }
      }
