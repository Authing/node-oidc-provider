...
module.exports = function getIdToken(provider) {
  return class IdToken {
    ...
    generateClientIssuer(providerIssuer, clientDomain) {
      let exp = /(https?:\/\/)([^.]*)/
    ...
    async sign({
      ...
      const signed = await (() => {
        ...
        let key = keystore && keystore.get({ alg, use: 'sig' });
        if(!key) {
          // 如果用户没有自定义 RSA 加密秘钥，就使用 OIDC Provider 默认的
          keystore = instance(provider).keystore;
          key = keystore && keystore.get({ alg, use: 'sig' });
        }
        let signOptions;
        if (use === 'introspection') {
          ...
          signOptions = {
            noIat: true, expiresIn: 60, audience: client.clientId, issuer: this.generateClientIssuer(provider.issuer, client.domain),
          };
        } else {
          signOptions = {
            ...
            issuer: this.generateClientIssuer(provider.issuer, client.domain),
            subject: payload.sub,
          };
        }
